...
export default function VisualizerPanel({
        ...
        return (
          <div className="w-full h-full flex items-center justify-center p-3">
            <BifoldVisualizer
              ...
              ventPositions={(config as BifoldConfig).ventPositions}
              view={view}
              ...
              transomBarPositions={(config as BifoldConfig).transomBarPositions}
              astragalBars={(config as BifoldConfig).astragalBars}
              astragalBarPositions={
                (config as BifoldConfig).astragalBarPositions
              }
            />
          </div>
        );
  ...
  return (
    <div
      className={
        className || "w-full xl:w-120 xl:shrink-0 xl:sticky xl:top-20 z-0"
      }
    >
      <div
        className={`bg-white rounded-xl overflow-hidden ${hideControls ? "" : "shadow-sm border border-slate-200"}`}
      >
        {/* Visualizer Preview */}
        <div
          className="relative"
        ...
        {!hideControls && onToggleView && (
          <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between bg-white">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-600 bg-slate-50 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 rounded-lg border border-slate-200 transition-all duration-200"
            >
