...
const RealisticFrenchDoorVisualizer: React.FC<RealisticFrenchDoorVisualizerProps> = ({
    config,
    width,
    height,
    view
    ...
    const frameColor = view === 'outside' ? config.outsideColor : config.insideColor;
    const { bg: frameBg, borderColor: frameBorderColor, sashBorder } = getFrameStyles(frameColor);
    const handleClass = getHandleStyle(config.hardwareColor);

    // Calculate sidelight dimensions
    const totalWidth = width;
    const leftSidelightWidth = config.sidelights.left.enabled ? config.sidelights.left.width : 0;
    const rightSidelightWidth = config.sidelights.right.enabled ? config.sidelights.right.width : 0;
        ...
        >
            ...
            <div className="absolute inset-0 p-4 flex flex-col pointer-events-none">
                ...
                `}>
                    ...
                    <div className={`absolute bottom-0 left-0 right-0 h-[4px] z-50 ${config.threshold === 'low' ? 'bg-gray-300' : 'bg-transparent'} border-t border-white/20`} />
                        ...
                        {config.sidelights.left.enabled && (
                            ...
                            >
                                ...
                                {config.sidelights.left.transom && (
                                    <div className={`absolute top-1/3 w-full h-[8px] ${frameBg}`} />
                                )}
                            </div>
                        )}
            ...
            <div className="absolute inset-0 p-4 pointer-events-none z-40">
                <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                    <defs>
                        <marker id="fd-arrowhead-right" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
                            <polygon points="0 0, 10 3.5, 0 7" fill="#ef4444" />
                        </marker>
                        <marker id="fd-arrowhead-left" markerWidth="10" markerHeight="7" refX="1" refY="3.5" orient="auto">
                    ...
                    {(() => {
                        ...
                        const lW = config.sidelights.left.enabled ? config.sidelights.left.width : 0;
                        const rW = config.sidelights.right.enabled ? config.sidelights.right.width : 0;
                        ...
                        return (
                            <>
                                ...
                                <line
                                    x1={d1Cx + arrowLen} y1={cy} x2={d1Cx - arrowLen} y2={cy}
                                    stroke="#ef4444" strokeWidth={strokeW} markerEnd="url(#fd-arrowhead-left)"
                                    style={{ filter: 'drop-shadow(0px 1px 2px white)' }}
                                />

                                {/* Right Door Arrow (Points Right — towards hinge) */}
                                <line
                                    x1={d2Cx - arrowLen} y1={cy} x2={d2Cx + arrowLen} y2={cy}
                                    stroke="#ef4444" strokeWidth={strokeW} markerEnd="url(#fd-arrowhead-right)"
                                    style={{ filter: 'drop-shadow(0px 1px 2px white)' }}
                                />
                            </>
                        );
