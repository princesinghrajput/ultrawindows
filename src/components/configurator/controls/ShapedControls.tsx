...
interface ShapedControlsProps {
    config: ShapedConfig;
    onUpdate: (updates: Partial<ShapedConfig>) => void;
}
...
export default function ShapedControls({ config, onUpdate }: ShapedControlsProps) {
    return (
        <>
            {/* Shape Type Selector */}
                    ...
                    {([
                        {
                            ...
                            icon: (
                                <svg viewBox="0 0 80 60" className="w-16 h-12">
                                    <polygon points="10,55 40,8 70,55" fill="#475569" stroke="#334155" strokeWidth="2" />
                                </svg>
                            ),
                        },
                        {
                            label: "Shaped Left", value: "shaped_left",
                            icon: (
                                <svg viewBox="0 0 80 60" className="w-16 h-12">
                                    <polygon points="10,55 10,30 55,8 70,55" fill="#475569" stroke="#334155" strokeWidth="2" />
                                </svg>
                            ),
                        },
                        {
                            label: "Shaped Right", value: "shaped_right",
                            icon: (
                                <svg viewBox="0 0 80 60" className="w-16 h-12">
                                    <polygon points="10,55 25,8 70,30 70,55" fill="#475569" stroke="#334155" strokeWidth="2" />
                                </svg>
                            ),
                        },
                        {
                            label: "Gable w/ Upstand", value: "gable_upstand",
                            icon: (
                                <svg viewBox="0 0 80 60" className="w-16 h-12">
                                    <polygon points="10,55 10,38 40,8 70,38 70,55" fill="#475569" stroke="#334155" strokeWidth="2" />
                                </svg>
                            ),
                        },
                    ] as { label: string; value: string; icon: React.ReactNode }[]).map((shape) => (
                        <button
                            key={shape.value}
                            onClick={() => onUpdate({ shapeType: shape.value } as any)}
                            className={`flex flex-col items-center justify-center p-4 border-2 rounded-xl w-28 h-28 transition-all ${config.shapeType === shape.value
            ...
            <SectionCard title="Cill">
                <div className="flex flex-wrap justify-center gap-3">
                    {CILL_OPTIONS.map((opt) => (
                        <button
                            key={opt.value}
                            onClick={() => onUpdate({ cill: opt.value } as any)}
