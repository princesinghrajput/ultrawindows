...
export function calculateBifoldPrice(config: {
    ...
    // 4. Trickle vents
    if (config.trickleVents) {
        items.push({
            label: 'Trickle Vents',
            quantity: 1,
            unitPrice: BIFOLD_PRICES.trickleVent,
            total: BIFOLD_PRICES.trickleVent,
        });
    }

    // 5. Custom RAL colour
    const standardColours = ['white', 'black', 'grey', 'anthracite'];
    const outsideColor = (config.outsideColor || config.color || 'white').toLowerCase();
    const insideColor = (config.insideColor || config.color || 'white').toLowerCase();
